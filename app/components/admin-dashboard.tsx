...
type User = { id: string; name: string; email: string; company: string; plan: Plan; role: string; status: Status; email_verified_at: string | null; created_at: string; active_sessions: number; usage_leads: number };
...
export function AdminDashboard() {
  ...
  return <main className="admin-shell"><header className="admin-header"><a className="brand" href="/"><span className="brand-mark">P</span><span>PROSPECTA WORBITA</span></a><div><a href="/conta/seguranca">Minha segurança</a><a href="/workspace">Plataforma</a></div></header><section className="admin-content"><div className="admin-title"><div><p className="eyebrow"><span /> Administração</p><h1>Contas e permissões</h1></div><p>Planos, estados e sessões são aplicados no servidor e registrados na auditoria.</p></div><form className="admin-search" onSubmit={search}><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar nome, empresa ou e-mail" /><button className="button button-primary">Buscar</button></form><p className="workspace-status">{message}</p><div className="admin-table"><table><thead><tr><th>Conta</th><th>Verificação</th><th>Plano</th><th>Estado</th><th>Sessões</th><th>Leads usados</th><th>Ação</th></tr></thead><tbody>{users.map((user) => <AdminUserRow key={user.id} user={user} onSave={update} />)}</tbody></table></div><section className="audit-panel"><h2>Auditoria recente</h2>{audits.slice(0, 20).map((audit) => <article key={audit.id}><strong>{audit.action}</strong><span>{audit.actor_email ?? "sistema"} → {audit.target_email ?? "—"}</span><time>{new Date(audit.created_at).toLocaleString("pt-BR")}</time></article>)}</section></section></main>;
}

function AdminUserRow({ user, onSave }: { user: User; onSave: (user: User, plan: Plan, status: Status) => Promise<void> }) {
  const [plan, setPlan] = useState(user.plan);
  const [status, setStatus] = useState(user.status);
  return <tr><td><strong>{user.name}</strong><small>{user.email}<br />{user.company || "Conta individual"}</small></td><td>{user.email_verified_at ? "Confirmado" : "Pendente"}</td><td><select value={plan} onChange={(event) => setPlan(event.target.value as Plan)}><option value="trial">Teste grátis</option><option value="essential">Essential</option><option value="growth">Growth</option><option value="scale">Scale</option></select></td><td><select value={status} onChange={(event) => setStatus(event.target.value as Status)}><option value="active">Ativa</option><option value="suspended">Suspensa</option><option value="cancelled">Cancelada</option><option value="deletion_requested">Exclusão solicitada</option></select></td><td>{user.active_sessions}</td><td>{user.usage_leads}</td><td><button type="button" onClick={() => void onSave(user, plan, status)}>Salvar</button></td></tr>;
}
