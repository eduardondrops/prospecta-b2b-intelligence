...
import { createOperationalAccessToken, currentUser } from "@/app/lib/auth";
...
export async function POST(request: Request) {
  ...
  try {
    const operationalAccess = await createOperationalAccessToken(user);
    const response = await fetch(`${appUrl.replace(/\/$/, "")}/api/bridge/access`, {
      method: "POST",
      headers: { Cookie: request.headers.get("cookie") ?? "", Accept: "application/json", "Content-Type": "application/json" },
      body: JSON.stringify({ token: operationalAccess.rawToken }),
    });
    const data = await response.json() as AccessResponse;
    if (!response.ok || !data.sucesso || data.token !== operationalAccess.rawToken) {
